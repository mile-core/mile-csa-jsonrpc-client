...
namespace milecsa {
    namespace rpc {
        ...
        std::optional<time_t> Client::ping() const {
            ...
            if(auto res = session->request(session->next_command("ping"),response_handler,error_handler)) {
        ...
        std::optional<uint256_t> Client::get_current_block_id() const {
            if(auto json = session->request(session->next_command("get-current-block-id"),response_handler,error_handler)){
        ...
        rpc::response Client::get_network_state() const {
            return session->request(session->next_command("get-network-state"),response_handler,error_handler);
        }

        rpc::response Client::get_nodes() const {
            return session->request(session->next_command("get-nodes"),response_handler,error_handler);
        }

        rpc::response Client::get_blockchain_info() const {
            return session->request(session->next_command("get-blockchain-info"),response_handler,error_handler);
        }

        rpc::response Client::get_blockchain_state() const {
            return session->request(session->next_command("get-blockchain-state"),response_handler,error_handler);
        }

        rpc::response Client::get_block(uint256_t id) const {
            json command = session->next_command("get-block-by-id", {{"id", id}});
            return session->request(command,response_handler,error_handler);
        }
